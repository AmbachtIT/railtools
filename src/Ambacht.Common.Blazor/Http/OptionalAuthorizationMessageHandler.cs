...
/// </summary>
public class OptionalAuthorizationMessageHandler : DelegatingHandler, IDisposable
{
	...
	protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
	{
		...
		if (request.RequestUri != null && _authorizedUris.Any(uri => uri.IsBaseOf(request.RequestUri)))
		{
			...
			{
				var token = await GetTokenFromStore();
				if (token != null)
				{
					_lastToken = token;
	...
	private async Task<AccessToken> GetTokenFromStore()
	{
	...
	public OptionalAuthorizationMessageHandler ConfigureHandler(
