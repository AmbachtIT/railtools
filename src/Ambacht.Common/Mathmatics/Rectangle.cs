...
namespace Ambacht.Common.Mathmatics
{
	...
	public record struct Rectangle<T> : IFormattable where T : IFloatingPoint<T>
	{
		...
		{
			return new Rectangle<T>(Left - amountX, Top - amountY, Width + (amountX + amountX),
				Height + (amountY + amountY));
		}
		...
		public Rectangle<T1> Cast<T1>() where T1 : IFloatingPoint<T1>
		{
			return new Rectangle<T1>(
				T1.CreateChecked(Left),
				T1.CreateChecked(Top),
				T1.CreateChecked(Width),
				T1.CreateChecked(Height));
		}
	}
