...
namespace Ambacht.Common.Time
{
	public class Day : Period
	{
		...
		public static DateTime? Parse(string str, bool local)
		{
			if (str != null && str.Length == 8)
			{
				...
				{
					if (year >= 1900 && year < 2500 && month >= 1 && month <= 12 && nr >= 1 && nr <= 31)
					{
						try
						{
						    return new DateTime(year, month, nr, 0, 0, 0, local ? DateTimeKind.Local : DateTimeKind.Utc);
						}
						catch(Exception)
						{
							return null;
	...
	public class DayPeriodType : PeriodType
	{
		...
		{
			var date = Time.Day.Parse(code, true);
			if(date.HasValue)
			{
				return new Day()
				       {
